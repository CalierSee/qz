...
//  aixianfeng
//
//  Created by 景彦铭 on 2016/11/15.
//  Copyright © 2016年 景彦铭. All rights reserved.
//

#import <Foundation/Foundation.h>

@interface QZSqliteManager : NSObject
//制定创建方法
...
/**
 检查数据库中是否存在对象所对应的数据记录并且判断给定的字段的值是否相等

 @param table 表名
 @param idField id字段
 @param field 要比较的字段
 @return 1<<0 数据不存在  1<<1 数据存在并且制定的字段的值都不相同  1<<2 数据存在并且指定的第一个字段值相同 1<<3 数据存在并且指定的第二个字段值相同
 */
//- (nonnull int)isExistsWithObject(nonnull id)objc withTable:(nonnull NSString *)table idField:(nonnull NSString *)idField field:(nullable NSString *)field,...NS_REQUIRES_NIL_TERMINATION;
@end
