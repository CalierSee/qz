...
@interface QZSqliteManager ()
{
    sqlite3 * _sql;
}
@end

@implementation QZSqliteManager
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        NSAssert(![self createSqlite], @"数据库打开失败");
    }
    return self;
}

- (int)createSqlite {
    NSLog(@"%s",[self sqlitePath]);
...
- (const char *)sqlitePath {
    return [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject stringByAppendingPathComponent:@"axf.db"].UTF8String;
}
